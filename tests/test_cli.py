...
def test_protocol_version_v9_now_accepted():
    """v9 is the version the 0.10.0 `render_removed` ops need."""
    with CLISession() as s:
...
def test_protocol_version_v7_now_accepted():
    """v7 is the version the 0.8.0 `trace_text` ops need."""
    with CLISession() as s:
...
def test_protocol_version_v2_still_accepted():
    """v2 clients keep working against a v5 subprocess (backward compat)."""
    with CLISession() as s:
