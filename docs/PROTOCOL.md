...
parent via length-prefixed JSON frames over stdin/stdout. **v0.11.3 ships
...
- Any of those values may be written as a reference to an object *(since
  0.11.2)*. An `/OCProperties` that lists no group (`/OCGs` empty or
  absent) may omit `/D` *(since 0.11.2)*: it has nothing to switch off. Its
  pages are still compared, so the answer comes from what MuPDF draws, not
